package pixel.utility
{
	...
	public class Object3DLoader
	{
		...
		//行结尾
		public static const ENTER:int = 0x0a;
		// "/"分隔符
		...
		private var _isZxy:Boolean = false;
		 ...
		 * 
		 **/
		public function load(stream:ByteArray,isZxy:Boolean = false,mirrorUV:Boolean = false):FileObj3D
		{
			_isZxy = isZxy;
			_mirrorUV = mirrorUV;
			//_objStream = stream;
		...
		private function parseVertex(pos:Array):void
		{
			if(_isZxy)
			{
				_obj3D.addVertex(pos[1]);
				_obj3D.addVertex(pos[2]);
				_obj3D.addVertex(pos[0]);
				//_obj3D.addVertex(pos.pop());
			}
			else
			{
				while(pos.length > 0)
		...
		private function parseVertexUV(pos:Array):void
		{
			if(_mirrorUV)
			{
				_obj3D.addVertexUV(Number(pos[0]));
				_obj3D.addVertexUV(1 - Number(pos[1]));
			}
			else
			{
				_obj3D.addVertexUV(1 - Number(pos[0]));
				_obj3D.addVertexUV(1 - Number(pos[1]));
			}
//			if(pos.length > 2)
//			{
...
//			}
		...
		private function parseIndex(codes:Array,group:FileObj3DVertexIndexGroup = null):void
		{
			...
			while(codes.length)
			{
				...
				if(code.indexOf("/") >= 0)
				{
					...
					_obj3D.addVertexIndex(int(subCodes[0]));
					_obj3D.addUVIndex(uint(subCodes[1]));
					//idx.uvIdx = int(subCodes[1]);
					if(subCodes.length > 2)
					{
						//idx.normalsIdx = int(subCodes[2]);
						_obj3D.addNormals(int(subCodes[2]));
					}
				}
				else
				{
					//idx.vertexIdx = int(code);
					_obj3D.addVertexIndex(uint(code));
				}
		...
		private function readLine(stream:ByteArray):String
		{
			...
			_length = stream.position - _streamPos;
			if(_length > 0)
			{
				stream.position = _streamPos;
